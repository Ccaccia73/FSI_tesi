...
In order to be flexible and easily implemented, the impact on the solvers should be as minimal as possible: for this reason, preCICE offers a high level \acrfull{api} (Section \ref{sec:pc-api}) different languages, such as C/C++, Fortran and Python.
...
between the participants (Section \ref{sec:pc-comm}), the mapping of data between meshes (Section \ref{sec:pc-map}). preCICE is configured by means of an \acrfull{xml} file (Section \ref{sec:pc-config}).
