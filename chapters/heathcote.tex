...
The root displacement is much smaller than the one used in the article in order to keep the mesh deformation small. Nevertheless the difference in the bending of the wing needs to be further analyzed.
...
The number of required iteration is, in this model, very stable: 15 iterations are required at each time step to converge, as shown in Figure~\ref{fig:hc-iterations}. As in the previous example, nodal forces converge slower than displacements.
...
\begin{figure}[htbp!]
	...
	\end{subfigure}
	\caption{forces on wing surface}
